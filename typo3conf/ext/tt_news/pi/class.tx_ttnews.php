...
require_once (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extPath('tt_news') . 'lib/class.tx_ttnews_catmenu.php');
require_once (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extPath('tt_news') . 'lib/class.tx_ttnews_helpers.php');
require_once (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extPath('tt_news') . 'lib/class.tx_ttnews_cache.php');
...
//				debug($selectConf, $this->theCode.' final $selectConf (' . __CLASS__ . '::' . __FUNCTION__ . ')', __LINE__, __FILE__, 3);
	...
	function getRelatedNewsByCategory(&$markerArray, $row, $lConf) {
		...
		$this->conf = \TYPO3\CMS\Core\Utility\GeneralUtility::array_merge_recursive_overrule($this->conf, $this->conf['relNewsByCategory.'] ? $this->conf['relNewsByCategory.'] : array());
	...
	function initGenericMarkers() {
		if (is_array($this->conf['genericmarkers.'])) {
			...
			if (is_array($this->genericMarkerConf[$this->theCode . '.'])) {
				$this->genericMarkerConf = \TYPO3\CMS\Core\Utility\GeneralUtility::array_merge_recursive_overrule($this->genericMarkerConf, $this->genericMarkerConf[$this->theCode . '.']);
			}
		}
	}
	...
	function getPrevNextLink($rec, $lConf, $p = 'prev') {
		$title = $rec['title'];
		$pTmp = $this->tsfe->ATagParams;
		$this->tsfe->ATagParams = $pTmp . ' title="' . $this->local_cObj->stdWrap(trim(htmlspecialchars($title)), $lConf[$p . 'LinkTitle_stdWrap.']) . '"';
...
//								'backPid' => null, $this->pointerName => null), $this->allowCaching, 0, $catSelLinkParams), $lConf[$titleWrap]);
	...
	function getRelatedNewsAsList($uid) {
		...
		$this->conf = \TYPO3\CMS\Core\Utility\GeneralUtility::array_merge_recursive_overrule($this->conf, $this->conf['displayRelated.']);
		$this->config = $this->conf;
		$this->arcExclusive = $this->conf['archive'];
		$this->LOCAL_LANG_loaded = FALSE;
	...
	function getRelated($uid) {
		...
		if ($res) {
			...
			$veryLocal_cObj = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('tslib_cObj'); // Local cObj.
			$lines = array();
	...
	function getRelatedPages($uid) {
		...
		$where = 'tt_news_related_mm.uid_local=' . $uid . '
					...
					AND tt_news_related_mm.tablenames=' . $this->db->fullQuoteStr('pages', 'tt_news_related_mm') . $this->getEnableFields('pages');
	 ...
	 *
	 * @param	int	$showResultCount	If set (default) the text "Displaying results..." will be shown, otherwise not.
...
debugster($cachingEngine);
			...
			switch ($this->confArr['cacheClearMode']) {
				...
				default : // normal
					if (method_exists($this->tsfe,'get_cache_timeout')) { // TYPO3 >= 4.2
						$lifetime = $this->tsfe->get_cache_timeout(); // seconds until a cached page is too old
					} else {
						$lifetime = 86400;
					}
					break;
	...
	function getLayouts($templateCode, $alternatingLayouts, $marker) {
		...
		if ($this->config['altLayoutsOptionSplit']) {
			$splitLayouts = $this->tsfe->tmpl->splitConfArray(array('ln' => $this->config['altLayoutsOptionSplit']), $this->config['limit']);
