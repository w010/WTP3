...
// for some reason it doesn't include this with autoload
require_once(\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extPath('tt_news').'lib/class.tx_ttnews_recordlist.php');
...
//		$this->CMD = GeneralUtility::_GP('CMD');
		...
		if ($this->confArr['useStoragePid']) {
			$tceTSC = BackendUtility::getTCEFORM_TSconfig('tt_news_cat',array('pid'=>$this->id));
		}
		...
		$newArticlePidPI = BackendUtility::readPageAccess($this->newArticlePid,$this->perms_clause);
...
//		}
	...
	function displayOverview() {
		...
		foreach($list as $pid => $stat)	{
			...
			if ($pa['path'])	{
				$tRows[] = '
					<tr class="bgColor4">
						<td><a href="'. BackendUtility::getModuleUrl('web_txttnewsM1', array('id' => $pid)).'">'.htmlspecialchars($pa['path']).'</a></td>
	...
	function getTreeObj() {
		...
		if (!is_object($this->treeObj)) {
			$this->treeObj = GeneralUtility::makeInstance('tx_ttnewscatmanager_treeView');
		}
		...
		$this->treeObj->returnUrl = BackendUtility::getModuleUrl('web_txttnewsM1', $urlparams);
		...
		foreach ($beUserSelCatArr as $catID) {
			$tmpR = BackendUtility::getRecord('tt_news_cat',$catID,'parent_category,hidden',$addWhere);
	...
	function displayNewsList()	{
		...
		$dblist = GeneralUtility::makeInstance('tx_ttnews_recordlist');
	...
	function processAjaxRequestConstruct() {
		...
		$SOBE->doc = GeneralUtility::makeInstance('template');
...
//			$reset = '<a href="'.$this->script.'?id='.$this->id.'" id="resetcatselection">'.$LANG->getLL('resetCatSelection').'</a>';
			...
			if ($this->useSubCategories && ($subCats = GeneralUtility::rmFromList($this->category,$this->selectedCategories))) {
				if (!$this->mData['showHiddenCategories']) {
	...
	function renderTreeCheckBoxes() {
		...
		foreach ($allowedCbNames as $n) {
			if ((bool)$show['cb_'.$n]) {
				$out[] = BackendUtility::getFuncCheck($params,'SET['.$n.']',$this->MOD_SETTINGS[$n],'','','id="cb-'.$n.'"').
		...
		foreach ($allowedCbNames as $n) {
			if ((bool)$show['cb_'.$n]) {
				$out[] = '<span class="list-cb">' .
						BackendUtility::getFuncCheck($params, 'SET['.$n.']', $this->MOD_SETTINGS[$n], '', '', 'id="cb-' . $n . '"') .
	...
	function renderNewCatButton() {
		...
		if ($this->mayUserEditCategories && (bool)$show['btn_newCategory'])	{
			...
			$onclick = htmlspecialchars(BackendUtility::editOnClick($params,$GLOBALS['BACK_PATH'],$this->returnUrl));
...
// 		}
		...
		if (isset($this->id)) {
			if ($GLOBALS['BE_USER']->check('modules','web_list'))	{
				...
				$href = BackendUtility::getModuleUrl('web_list', array ('id' => $this->pageinfo['uid'], 'returnUrl' => GeneralUtility::getIndpEnv('REQUEST_URI')) );
				...
				// View
			$buttons['view'] = '<a href="#" onclick="' . htmlspecialchars(BackendUtility::viewOnClick($this->id, $backPath, BackendUtility::BEgetRootLine($this->id))) . '">' .
			...
			if ($this->localCalcPerms&2 && !empty($this->id))	{
				...
				$buttons['edit'] = '<a href="#" onclick="' . htmlspecialchars(BackendUtility::editOnClick($params, $backPath, -1)) . '">' .
...
//			}
			...
			if ($GLOBALS['BE_USER']->mayMakeShortcut()) {
				...
				// Back
			if ($this->returnUrl) {
				$buttons['back'] = '<a href="' . htmlspecialchars(GeneralUtility::linkThisUrl($this->returnUrl, array('id' => $this->id))) . '" class="typo3-goBack">' .
	...
	function getLangMenu() {
		...
		if (count($this->MOD_MENU['language'])>1) {
			$menu = $GLOBALS['LANG']->sL('LLL:EXT:lang/locallang_general.xml:LGL.language',1) .
				BackendUtility::getFuncMenu($this->id,'SET[language]',$this->current_sys_language,$this->MOD_MENU['language']);
		}
		return $menu;
	}
	...
	function getPageSelector() {
		...
		if (count($this->MOD_MENU['searchLevels'])>1) {
			$menu = $GLOBALS['LANG']->getLL('enterSearchLevels') .
				BackendUtility::getFuncMenu($this->id,'SET[searchLevels]',$this->searchLevels,$this->MOD_MENU['searchLevels']);
		}
	 ...
	 */
	function initGPvars() {
		$this->pointer = $this->compatibility()->intInRange(GeneralUtility::_GP('pointer'),0,100000);
...
class tx_ttnewscatmanager_treeView extends tx_ttnews_categorytree {
	...
	function makeControl($table,$row)	{
		...
		if ($this->mayUserEditCategories)	{
			...
			$cells[]='<a href="#" onclick="'.htmlspecialchars(BackendUtility::editOnClick($params,$this->backPath,$this->returnUrl)).'">'.
...
// 				'</a>';
	...
	function issueCommand($params,$rUrl='')	{
		$rUrl = $rUrl ? $rUrl : GeneralUtility::getIndpEnv('REQUEST_URI');
		return $this->backPath.'tce_db.php?'.
				...
				'&prErr=1&uPT=1'. BackendUtility::getUrlToken('tceAction');
	}
}
...
if (!(TYPO3_REQUESTTYPE & TYPO3_REQUESTTYPE_AJAX)) {
	$SOBE = GeneralUtility::makeInstance('tx_ttnews_module1');
	$SOBE->init();
