...
class FrontendLoginController extends \TYPO3\CMS\Frontend\Plugin\AbstractPlugin {
	...
	protected function showLogin() {
		...
		if ($this->conf['showForgotPasswordLink']) {
			...
			$linkpartArray['###FORGOT_PASSWORD_LINK###'] = explode('|', $this->getPageLink('|', array($this->prefixId . '[forgot]' => 1), false, $this->conf['pid_forget']));
			// wolo mod end.
	 ...
	 */
	protected function getPageLink($label, $piVars, $returnUrl = FALSE, $pid = null) {
		$additionalParams = '';
		if (count($piVars)) {
		...
		$this->conf['linkConfig.']['parameter'] = intval($pid) ? intval($pid) : $GLOBALS['TSFE']->id;
		// wolo mod end.
