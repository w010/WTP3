...
die('stop');*/
		...
		return array(
			// wolo mod - save selected groups uids to display in stats and details
			'queryInfo' => array('groups_selected' => implode(',', $groups), 'id_lists' => $id_lists)
		);
	}
	...
	function makeListDMail($boxID, $totalBox, $open=FALSE){
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
			...
			// wolo mod - display drafts on main list
				' AND scheduled=0'.BackendUtility::deleteClause('sys_dmail'),
			'',
		...
		$output.= DirectMailUtility::formatTable($tblLines, array(), 1, array(1,1,1,1,0,0,1,0,1), 'border="0" cellspacing="0" cellpadding="3"');
		$output.= '</div></div>';
		return $output;
	}
