...
//	'dontShowCommands' => defined('LOCAL') && LOCAL ? 0 :   !DEV?1: 0,
	...
	'docker' => true,
	// todo: this must be an option
	'docker_containerSql' => 'berglanddev_mysql_1'
];
...
class Dump  {
	...
	function main() {
		...
		if (!$_POST['submit'] && !$_POST['name'])    list($this->projectName) = preg_split('@\.@', $_SERVER['HTTP_HOST']);

		//$projectDirectory = $this->projectName?$this->projectName:'public_html';
		/*if (!is_dir(PATH_site.$projectDirectory.'/'))	{
			msg('error: no "'.$projectDirectory.'" directory found in PATH_site (not running from root?)');
			return;
		}*/
...
<footer>
	<i>WTP DUMP/BACKUP TOOL v<?php print SCRIPT_VERSION; ?> - wolo.pl '.' studio 2013-2017</i>
</footer>
